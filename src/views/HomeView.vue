...
// Init
onMounted(async () => {
  // Charger lieux puis tags locaux
  await lieuxStore.loadLieux();

  // Charger tags depuis localStorage immédiatement (permet persistance hors connexion)
  loadTagsLocal();

  // Sur changement d'auth (après mount), si utilisateur connecté, synchroniser depuis Firestore
  onAuthStateChanged(auth, (user) => {
    if (user) {
      // récupère et fusionne les tags Firestore
      fetchTagsFromFirestore();
    } else {
      // ne pas vider availableTags pour garder la persistance locale
      console.log("Utilisateur déconnecté — utilisation des tags locaux.");
    }
  });
